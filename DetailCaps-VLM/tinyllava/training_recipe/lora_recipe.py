...
class LoRATrainingRecipe(BaseTrainingRecipe):
    ...
    def save(self, model, trainer):
        ...
        #save MSCORCH-model
        MSCORCH_state_dict = get_peft_state_non_lora_maybe_zero_3(model.vision_tower.MSCORCH.named_parameters(), False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            MSCORCH_output_dir = os.path.join(self.training_arguments.output_dir, 'MSCORCH')
            os.makedirs(MSCORCH_output_dir, exist_ok=True)
            MSCORCH_output_path = os.path.join(self.training_arguments.output_dir, 'MSCORCH/pytorch_model.bin')
            torch.save(MSCORCH_state_dict, MSCORCH_output_path)
        #save SSA-model
        SSA_state_dict = get_peft_state_non_lora_maybe_zero_3(model.vision_tower.SSA.named_parameters(), False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
        ...
        #save DSVAB-model
        DSVAB_state_dict = get_peft_state_non_lora_maybe_zero_3(model.vision_tower.DSVAB.named_parameters(), False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
