...
class BaseTrainingRecipe:
    ...
    def add_args(self, model_args):
        ...
        if self.training_arguments.pretrained_model_path is not None:
            ...
            model_args['MSCORCH'] = dict(pretrained_MSCORCH_path=os.path.join(self.training_arguments.pretrained_model_path, 'MSCORCH'))
            model_args['SSA'] = dict(pretrained_SSA_path=os.path.join(self.training_arguments.pretrained_model_path, 'SSA'))
            model_args['DSVAB'] = dict(pretrained_DSVAB_path=os.path.join(self.training_arguments.pretrained_model_path, 'DSVAB'))
        return model_args
    ...
    def _vision_tower_tune_type_setting(self, model):
        ...
        if tune_type == 'full':
            ...
            model.vision_tower.resnet50.requires_grad_(False)
        elif tune_type == 'frozen':
    ...
    def save(self, model, trainer):
       ...
       #save MSCORCH
        MSCORCH_state_dict = get_state_maybe_zero_3(model.vision_tower.MSCORCH.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            MSCORCH_output_dir = os.path.join(self.training_arguments.output_dir, 'MSCORCH')
            os.makedirs(MSCORCH_output_dir, exist_ok=True)
            MSCORCH = os.path.join(self.training_arguments.output_dir, 'MSCORCH/pytorch_model.bin')
            torch.save(MSCORCH_state_dict, MSCORCH_output_path)
       #save SSA
        SSA_state_dict = get_state_maybe_zero_3(model.vision_tower.SSA.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            SSA_output_dir = os.path.join(self.training_arguments.output_dir, 'SSA')
            os.makedirs(SSA_output_dir, exist_ok=True)
            SSA_output_path = os.path.join(self.training_arguments.output_dir, 'SSA/pytorch_model.bin')
            torch.save(SSA_state_dict, SSA_output_path)
       #save DSVAB
        DSVAB_state_dict = get_state_maybe_zero_3(model.vision_tower.DSVAB.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            DSVAB_output_dir = os.path.join(self.training_arguments.output_dir, 'DSVAB')
            os.makedirs(DSVAB_output_dir, exist_ok=True)
            DSVAB_output_path = os.path.join(self.training_arguments.output_dir, 'DSVAB/pytorch_model.bin')
            torch.save(DSVAB_state_dict, DSVAB_output_path)
    ...
    def load(self, model, model_args={}):
        if not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
            ...
            model.load_MSCORCH(**model_args['MSCORCH'])
            model.load_SSA(**model_args['SSA']) 
            model.load_DSVAB(**model_args['DSVAB'])
        else:
