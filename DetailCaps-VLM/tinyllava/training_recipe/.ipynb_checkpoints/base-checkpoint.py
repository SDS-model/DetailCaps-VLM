...
class BaseTrainingRecipe:
    ...
    def add_args(self, model_args):
        ...
        if self.training_arguments.pretrained_model_path is not None:
            ...
            model_args['EnhancedCMN'] = dict(pretrained_EnhancedCMN_path=os.path.join(self.training_arguments.pretrained_model_path, 'EnhancedCMN'))
            model_args['GlobalFeatureEnhancement'] = dict(pretrained_GlobalFeatureEnhancement_path=os.path.join(self.training_arguments.pretrained_model_path, 'GlobalFeatureEnhancement'))
            model_args['MultiViewFeatureFusion'] = dict(pretrained_MultiViewFeatureFusion_path=os.path.join(self.training_arguments.pretrained_model_path, 'MultiViewFeatureFusion'))
        return model_args
    ...
    def save(self, model, trainer):
        ...
        #save trainer
        trainer.save_state()
        print(self.training_arguments.output_dir)
       ...
       #save EnhancedCMN
        EnhancedCMN_state_dict = get_state_maybe_zero_3(model.vision_tower.EnhancedCMN.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
       ...
       #save GlobalFeatureEnhancement
        GlobalFeatureEnhancement_state_dict = get_state_maybe_zero_3(model.vision_tower.GlobalFeatureEnhancement.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
       ...
       #save MultiViewFeatureFusion
        MultiViewFeatureFusion_state_dict = get_state_maybe_zero_3(model.vision_tower.MultiViewFeatureFusion.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
    ...
    def load(self, model, model_args={}):
        if not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
            ...
            model.load_EnhancedCMN(**model_args['EnhancedCMN'])
            model.load_GlobalFeatureEnhancement(**model_args['GlobalFeatureEnhancement']) 
            model.load_MultiViewFeatureFusion(**model_args['MultiViewFeatureFusion'])
        else:
