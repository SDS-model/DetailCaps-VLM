...
def load_pretrained_model(model_name_or_path, load_type='hf', load_8bit=False, load_4bit=False, device_map="auto",
    ...
    if model_name_or_path is not None and 'lora' not in model_name_or_path:
        ...
        if os.path.exists(os.path.join(model_name_or_path, 'adapter_config.json')):
            ...
            MSCORCH_ckp_path = os.path.join(model_name_or_path, 'MSCORCH/pytorch_model.bin')
            MSCORCH_ckp = load_base_ckp_for_lora(MSCORCH_ckp_path)
            model.vision_tower.MSCORCH.load_state_dict(MSCORCH_ckp)
            SSA_ckp_path = os.path.join(model_name_or_path, 'SSA/pytorch_model.bin')
            SSA_ckp = load_base_ckp_for_lora(SSA_ckp_path)
            model.vision_tower.SSA.load_state_dict(SSA_ckp)
            DSVAB_ckp_path = os.path.join(model_name_or_path, 'DSVAB/pytorch_model.bin')
            DSVAB_ckp = load_base_ckp_for_lora(DSVAB_ckp_path)
            model.vision_tower.DSVAB.load_state_dict(DSVAB_ckp)
