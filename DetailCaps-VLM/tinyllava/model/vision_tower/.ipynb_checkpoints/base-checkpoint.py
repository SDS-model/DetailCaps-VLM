...
class EnhancedCMN(nn.Module):
    def __init__(self, out_channels, norm_layer=None, fpn_inchannels=[256, 512, 1024],  # 修改输入通道
        ...
        # 调整SAB模块
        self.sab_d4 = SAB(fpn_dim, fpn_dim, fpn_dim//4)  # 处理f4层
        self.sab_d3 = SAB(fpn_dim, fpn_dim, fpn_dim//4)  # 处理f3层
        self.sab_d2 = SAB(fpn_dim, fpn_dim, fpn_dim//4)  # 处理f2层

        # 调整CFB模块数量
        self.cfb_d4 = nn.ModuleList([CFB(fpn_dim, fpn_dim//4, norm_layer) for _ in range(2)])
        self.cfb_d3 = nn.ModuleList([CFB(fpn_dim, fpn_dim//4, norm_layer) for _ in range(2)])
        self.cfb_d2 = nn.ModuleList([CFB(fpn_dim, fpn_dim//4, norm_layer), Identity()])
    ...
    def forward(self, f2, f3, f4):  # 输入改为三个层
        # 处理f4层
        feat = self.e4conv(f4)
        descriptors = [None, None, self.sab_d4(feat)]  # 初始化描述符
        ...
        for i in reversed(range(2)):  # 处理两个层（索引1和0）
            ...
            # 更新描述符
            if i == 1:
                descriptors[1] = self.sab_d3(feat)
            else:
                descriptors[0] = self.sab_d2(feat)
...
class VisionTower(nn.Module):
    def __init__(self, cfg):
        ...
        self.EnhancedCMN = EnhancedCMN(out_channels=512, norm_layer=lambda channels: nn.GroupNorm(num_groups=32, num_channels=channels)).to('cuda')  # 对齐到f3的尺寸
        self.GlobalFeatureEnhancement = GlobalFeatureEnhancement().to(device = 'cuda:0')
        self.MultiViewFeatureFusion = MultiViewFeatureFusion().to(device = 'cuda:0')
    ...
    def load_model_EnhancedCMN(self,**kwargs):
        pretrained_EnhancedCMN_path = get_value_from_kwargs(kwargs, 'pretrained_EnhancedCMN_path')
        EnhancedCMN_weights = torch.load(os.path.join(pretrained_EnhancedCMN_path, 'pytorch_model.bin'), map_location='cpu')
        def get_w(weights, keyword):
            return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
        self.EnhancedCMN.load_state_dict(EnhancedCMN_weights)
    def load_model_GlobalFeatureEnhancement(self,**kwargs):
        pretrained_GlobalFeatureEnhancement_path = get_value_from_kwargs(kwargs, 'pretrained_GlobalFeatureEnhancement_path')
        GlobalFeatureEnhancement_weights = torch.load(os.path.join(pretrained_GlobalFeatureEnhancement_path, 'pytorch_model.bin'), map_location='cpu')
        def get_w(weights, keyword):
            return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
        self.GlobalFeatureEnhancement.load_state_dict(GlobalFeatureEnhancement_weights)
    def load_model_MultiViewFeatureFusion(self,**kwargs):
        pretrained_MultiViewFeatureFusion_path = get_value_from_kwargs(kwargs, 'pretrained_MultiViewFeatureFusion_path')
        MultiViewFeatureFusion_weights = torch.load(os.path.join(pretrained_MultiViewFeatureFusion_path, 'pytorch_model.bin'), map_location='cpu')
        def get_w(weights, keyword):
            return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
        self.MultiViewFeatureFusion.load_state_dict(MultiViewFeatureFusion_weights)
    ...
    def forward(self, x, **kwargs):
        ...
        global_features = self.GlobalFeatureEnhancement(image_features)
        concepts = self.EnhancedCMN(images_features_res_c2, images_features_res_c3, images_features_res_c4)
        global_features = global_features.transpose(0,1)
        concepts = concepts.view(-1,84*84,512)
        concepts = concepts.transpose(0,1)
        fused_feature = self.MultiViewFeatureFusion(global_features,concepts)
