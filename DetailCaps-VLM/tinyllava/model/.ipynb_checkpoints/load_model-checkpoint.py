...
def load_pretrained_model(model_name_or_path, load_type='hf', load_8bit=False, load_4bit=False, device_map="auto",
    ...
    if model_name_or_path is not None and 'lora' not in model_name_or_path:
        ...
        if os.path.exists(os.path.join(model_name_or_path, 'adapter_config.json')):
            ...
            EnhancedCMN_ckp_path = os.path.join(model_name_or_path, 'EnhancedCMN/pytorch_model.bin')
            EnhancedCMN_ckp = load_base_ckp_for_lora(EnhancedCMN_ckp_path)
            model.vision_tower.EnhancedCMN.load_state_dict(EnhancedCMN_ckp)
            GlobalFeatureEnhancement_ckp_path = os.path.join(model_name_or_path, 'GlobalFeatureEnhancement/pytorch_model.bin')
            GlobalFeatureEnhancement_ckp = load_base_ckp_for_lora(GlobalFeatureEnhancement_ckp_path)
            model.vision_tower.GlobalFeatureEnhancement.load_state_dict(GlobalFeatureEnhancement_ckp)
